...
- [Codex 宠物实现笔记](docs/codex-pet-implementation.md)
- [宠物动画契约](docs/pet-animation-contract.json)
- [宠物动作提示词模板](docs/pet-authoring-prompts.md)
- [`.pet-runs` 阶段产物规范](docs/pet-run-layout.md)
...
```text
http://127.0.0.1:18091/preview/web/
```

仓库内置的示例宠物包位于 `examples/pets/lulu/`。
...
这会生成带图标的 `dist/LightPet.app`，使用 ad-hoc 签名，适合自己或少量朋友本机使用。`.app` 不内置宠物资源，仍读取 `${CODEX_HOME:-$HOME/.codex}/pets`。如果通过聊天工具或浏览器传给朋友，macOS 可能加上 quarantine 标记；这种非公证包需要右键打开一次，或由使用者自行移除 quarantine。
